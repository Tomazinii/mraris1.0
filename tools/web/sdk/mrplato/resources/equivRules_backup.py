...
def mappingGenerate(dic,premisses,lines):
    ...
    if lp != len(lines):
        ...
        perm = permutations(lines)  # Generates all possible combinations
        msg = ''
        # for p in perm:
        #     for lp in p:
        #         print(f'lp: {lp} - type: { type(lp)} ')
        for l in perm:
            # print(f'l2: {l} - type: { type(l)} ')
            dic = copy.copy(saved_dic)  # Restores the saved context (dictionary)
            r = True
            i = 0
            while i < lp:
        ...
        if not r:
            msg = 'Premisses of the rule and the line could not be unified.'
        return r, msg, dic
